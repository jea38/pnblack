...
$stmt = $pdo->prepare('SELECT * FROM lsaccounts WHERE email = ?');
...
if ($account) {
    ...
    // lsaccounts doesn't exist, so create one
    $stmt = $pdo->prepare('INSERT INTO lsaccounts (email, password, full_name, role, last_seen, registered) VALUES (?, ?, ?, ?, ?, ?)');
