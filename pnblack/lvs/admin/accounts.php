...
$stmt = $pdo->prepare('SELECT COUNT(*) AS total FROM lsaccounts a ' . $where);
if ($search) $stmt->bindParam('search', $param3, PDO::PARAM_STR);
$stmt->execute();
$accounts_total = $stmt->fetchColumn();
// SQL query to get all lsaccounts from the "accounts" table
$stmt = $pdo->prepare('SELECT a.*, (SELECT COUNT(*) FROM messages WHERE account_id = a.id) AS messages_total FROM lsaccounts a ' . $where . ' ORDER BY ' . $order_by . ' ' . $order . ' LIMIT :start_results,:num_results');
// Bind params
