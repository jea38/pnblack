...
if (isset($_GET['id'])) {
    ...
    if ($_SESSION['chat_account_role'] == 'Admin') {
        ...
        $stmt = $pdo->prepare('SELECT * FROM lsaccounts WHERE id = ? AND role != "Admin"');
    }
    ...
    if (isset($_POST['submit']) && $acc) {
        ...
        if ($_SESSION['chat_account_role'] == 'Admin') {
            $stmt = $pdo->prepare('UPDATE lsaccounts SET email = ?, password = ?, role = ?, full_name = ?, secret = ?, last_seen = ?, status = ?, photo_url = ?, ip = ?, user_agent = ?, registered = ? WHERE id = ?');
            ...
            $stmt = $pdo->prepare('UPDATE lsaccounts SET email = ?, password = ?, full_name = ?, secret = ?, last_seen = ?, status = ?, photo_url = ? WHERE id = ?');
    ...
    if (isset($_POST['delete']) && $_SESSION['chat_account_role'] == 'Admin') {
        // Delete the account
        $stmt = $pdo->prepare('DELETE FROM lsaccounts WHERE id = ?');
        $stmt->execute([ $_GET['id'] ]);
    ...
    if (isset($_POST['submit'])) {
        ...
        $stmt = $pdo->prepare('INSERT INTO lsaccounts (email,password,role,full_name,secret,last_seen,status,photo_url,ip,user_agent,registered) VALUES (?,?,?,?,?,?,?,?,?,?,?)');
