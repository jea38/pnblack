...
$stmt = $pdo->prepare('SELECT COUNT(*) AS total FROM conversations c LEFT JOIN lsaccounts a ON a.id = c.account_sender_id LEFT JOIN lsaccounts a2 ON a2.id = c.account_receiver_id WHERE (c.account_sender_id = :acc_id OR c.account_receiver_id = :acc_id) ' . $where);
...
$stmt = $pdo->prepare('SELECT c.*, (SELECT COUNT(*) FROM messages WHERE conversation_id = c.id) AS messages_total, a.full_name AS account_sender_full_name, a.email AS account_sender_email, a2.full_name AS account_receiver_full_name, a2.email AS account_receiver_email FROM conversations c LEFT JOIN lsaccounts a ON a.id = c.account_sender_id LEFT JOIN lsaccounts a2 ON a2.id = c.account_receiver_id WHERE (c.account_sender_id = :acc_id OR c.account_receiver_id = :acc_id) ' . $where . ' ORDER BY ' . $order_by . ' ' . $order . ' LIMIT :start_results,:num_results');
// Bind params
