...
if ($_SESSION['chat_account_role'] == 'Admin') {
    $stmt = $pdo->prepare('SELECT c.*, m.msg, a.full_name AS account_sender_full_name, a2.full_name AS account_receiver_full_name, a.email AS account_sender_email, a2.email AS account_receiver_email FROM conversations c JOIN lsaccounts a ON a.id = c.account_sender_id JOIN lsaccounts a2 ON a2.id = c.account_receiver_id LEFT JOIN messages m ON m.conversation_id = c.id WHERE c.id = ?');
...
$stmt = $pdo->prepare('SELECT m.*, a.* FROM messages m LEFT JOIN lsaccounts a ON a.id = m.account_id WHERE m.conversation_id = ? ORDER BY m.submit_date ASC');
$stmt->execute([ $_GET['id'] ]);
