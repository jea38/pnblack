...
if (isset($_GET['action']) && $_GET['action'] == 'conversation') {
    ...
    if (isset($_GET['id'])) {
        ...
        $stmt = $pdo->prepare('SELECT c.*, m.msg, a.full_name AS account_sender_full_name, a2.full_name AS account_receiver_full_name FROM conversations c JOIN lsaccounts a ON a.id = c.account_sender_id JOIN lsaccounts a2 ON a2.id = c.account_receiver_id LEFT JOIN messages m ON m.conversation_id = c.id WHERE c.id = ? AND (c.account_sender_id = ? OR c.account_receiver_id = ?) AND c.status = "Open"');
...
if (isset($_GET['action'], $_GET['id']) && $_GET['action'] == 'conversation_create') {
    ...
    $stmt = $pdo->prepare('SELECT * FROM lsaccounts WHERE id = ?');
    $stmt->execute([ $_GET['id'] ]);
...
if (isset($_GET['action']) && $_GET['action'] == 'message') {
    ...
    // Update status
    $stmt = $pdo->prepare('UPDATE lsaccounts SET status = "Occupied" WHERE id = ?');
...
if (isset($_GET['action']) && $_GET['action'] == 'info') {
    ...
    $stmt = $pdo->prepare('SELECT COUNT(*) AS total FROM lsaccounts WHERE last_seen > date_sub(?, interval 5 minute)');
    $stmt->execute([ date('Y-m-d H:i:s') ]);
    $accounts_total = $stmt->fetchColumn();
    // Retrieve the total number of requests
    $stmt = $pdo->prepare('SELECT COUNT(*) AS total FROM lsaccounts WHERE status = "Waiting"');
    $stmt->execute();
...
if (isset($_GET['action'], $_GET['id']) && $_GET['action'] == 'request') {
    ...
    $stmt = $pdo->prepare('SELECT * FROM lsaccounts WHERE status = "Waiting" AND id = ?');
    $stmt->execute([ $_GET['id'] ]);
    if ($stmt->fetch(PDO::FETCH_ASSOC)) {
        // Account is waiting, so update the account status to Idle
        $stmt = $pdo->prepare('UPDATE lsaccounts SET status = "Idle" WHERE id = ?');
        $stmt->execute([ $_GET['id'] ]);
...
if (isset($_GET['action'], $_GET['id']) && $_GET['action'] == 'request_delete') {
    ...
    $stmt = $pdo->prepare('UPDATE lsaccounts SET status = "Idle" WHERE id = ? AND status = "Waiting"');
    $stmt->execute([ $_GET['id'] ]);
    // Ouput success    
    exit('{"msg":"Success"}');
}
...
if (isset($_GET['action'], $_GET['status']) && $_GET['action'] == 'update_status') {
    // Update the account status
    $stmt = $pdo->prepare('UPDATE lsaccounts SET status = ? WHERE id = ?');
