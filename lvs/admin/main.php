...
$stmt = $pdo->prepare('UPDATE lsaccounts SET last_seen = ? WHERE id = ?');
...
function template_admin_header($title, $selected = 'orders', $selected_child = 'view') {
    global $pdo, $account;
    // Retrieve the total number of lsaccounts in the last 5 mins
    ...
    $admin_links = '
        ...
        <div class="sub">
            ...
            ' . ($_SESSION['chat_account_role'] == 'Admin' ? '<a href="account.php"' . ($selected == 'accounts' && $selected_child == 'manage' ? ' class="selected"' : '') . '><span>&#9724;</span>Create Account</a>' : '') . '
        </div>
    ';
